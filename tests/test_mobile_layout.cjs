...
test('short-landscape practice stage-middle densifies after cpu-row hide (#101)', () => {
  ...
  // Practice densify via :has(cpu-row[hidden]) — reclaim strip; battle keeps cpu-row.
  assert.match(css, /\.nr-stage-middle:has\(> \.nr-cpu-row\[hidden\]\) \{[\s\S]*?justify-content: space-between/);
  ...
  // Extreme-short keeps densify; battle ≤360 grid intact.
  assert.match(css, /@media \(orientation: landscape\) and \(max-height: 360px\)[\s\S]*?\.nr-stage-middle:has\(> \.nr-cpu-row\[hidden\]\) > \.timed-chains/);
  assert.match(css, /#modern-app\[data-kind='battle'\] \.nr-stage-middle \{ grid-template-columns: 1fr 1fr/);
  // JS still owns hide (drives :has).
  ...
  // Hint ≥44px path from #97/#21 still present.
  assert.match(css, /\.nr-stage-middle > \.nr-hint \{[\s\S]*?min-height: 44px; min-width: 44px/);
});
