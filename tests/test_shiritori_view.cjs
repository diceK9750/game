...
test('shiritori miss outline CSS mirrors numbers durable wrong feedback', () => {
  ...
  assert.match(player, /modern-ui\.css\?v=toolbar-tap-109-1/);
});

test('polite live region announces durable miss guidance; visual status stays non-live', () => {
  const {view, state} = harness();
  const live = view.page.querySelector('.nr-sr-only');
  const status = view.page.querySelector('.sh-status');
  assert.ok(live, 'polite status live region present');
  assert.equal(live.role, 'status');
  assert.equal(live['aria-live'], 'polite');
  assert.equal(live['aria-atomic'], 'true');
  // Visual strip must stay non-live so SR hear one channel (#98 parity; #65 outline separate).
  assert.equal(status.role, undefined);
  assert.equal(status['aria-live'], undefined);
  view.update({...state, required: 'り', completed: 2, mistakes: 0, miss_card: null,
    message: '「り」から始まる絵をさがそう。'});
  assert.match(live.textContent, /次は「り」/);
  assert.match(live.textContent, /2枚つながった/);
  assert.doesNotMatch(live.textContent, /ちがう絵/);
  view.update({...state, required: 'り', completed: 2, mistakes: 1, miss_card: 3,
    message: 'この絵は「り」につながらないよ（使用済み・ん終わりも不可）。 −3秒'});
  assert.match(status.textContent, /この絵は「り」につながらないよ/);
  assert.match(live.textContent, /ちがう絵！お題を確認して、すぐ押し直そう/);
  assert.match(live.textContent, /ミス1回/);
  assert.equal(view.page.querySelectorAll('.sh-card')[3].dataset.feedback, 'wrong');
  // Cleared miss_card drops guidance from live; outline TTL path covered elsewhere.
  view.update({...state, required: 'り', completed: 2, mistakes: 1, miss_card: null,
    message: '「り」から始まる絵をさがそう。'});
  assert.doesNotMatch(live.textContent, /ちがう絵/);
  assert.match(live.textContent, /次は「り」/);
});

test('wrong-card miss outline stays non-live while statusLive carries guidance (#65/#98)', () => {
  const {view, state} = harness();
  view.update({...state, miss_card: 1, mistakes: 1, message: 'この絵は「り」につながらないよ'});
  const card = view.page.querySelectorAll('.sh-card')[1];
  const live = view.page.querySelector('.nr-sr-only');
  assert.equal(card.dataset.feedback, 'wrong');
  assert.equal(card['aria-live'], undefined);
  assert.equal(card.role, undefined);
  assert.match(live.textContent, /ちがう絵！お題を確認して、すぐ押し直そう/);
  const player = fs.readFileSync(require.resolve('../player.html'), 'utf8');
  const index = fs.readFileSync(require.resolve('../index.html'), 'utf8');
  assert.match(player, /shiritori-ui\.js\?v=sh-miss-live-99-1/);
  assert.match(index, /player\.html\?v=toolbar-tap-109-1/);
});
...
test('shiritori solo result-cast denser RIN+score after LUNA hide (#84 parity #82)', () => {
  ...
  assert.match(player, /shiritori-ui\.css\?v=sh-solo-cast-84-1/);
  assert.match(player, /shiritori-ui\.js\?v=sh-miss-live-99-1/);
  assert.match(player, /mobile-layout\.css\?v=toolbar-tap-109-1/);
  assert.match(index, /player\.html\?v=toolbar-tap-109-1/);
...
test('shiritori ready solo hero-cast denser/centered after LUNA+VS hide (parity #82/#84; #89)', () => {
  const modern = fs.readFileSync(require.resolve('../modern-ui.css'), 'utf8');
  const mobile = fs.readFileSync(require.resolve('../mobile-layout.css'), 'utf8');
  const character = fs.readFileSync(require.resolve('../character-layout.css'), 'utf8');
  const player = fs.readFileSync(require.resolve('../player.html'), 'utf8');
  const index = fs.readFileSync(require.resolve('../index.html'), 'utf8');
  // Desktop + short-height + mobile denser/center (parity result solo-cast #82/#84).
  assert.match(modern, /\.nr-hero-cast:has\(> \.nr-koh\[hidden\]\) \{ gap: 12px; justify-content: center; \}/);
  assert.match(modern, /\.nr-hero-cast:has\(> \.nr-koh\[hidden\]\) \.nr-character \{ width: min\(56%, 280px\); \}/);
  assert.match(modern, /\.nr-hero-cast:has\(> \.nr-koh\[hidden\]\) \{ gap: 6px; justify-content: center; \}/);
  assert.match(mobile, /\.nr-hero-cast:has\(> \.nr-koh\[hidden\]\) \{ gap: 2px; justify-content: center; \}/);
  assert.match(mobile, /\.nr-hero-cast:has\(> \.nr-koh\[hidden\]\) \.nr-character \{ width: clamp\(48px, 14dvh, 96px\); \}/);
  ...
  assert.equal(rinIn(heroCast()).hidden, false, 'battle ready still shows RIN');

  view.update({...state, phase: 'intro', mode: 'solo', cards: []});
  assert.equal(kohIn(heroCast()).hidden, true, 'solo ready still hides LUNA (#88)');
  assert.equal(versusIn(heroCast()).hidden, true, 'solo ready still hides VS (#88)');
  assert.equal(rinIn(heroCast()).hidden, false, 'solo ready keeps RIN');

  // Intro entry focus still heading h1 (#56); leave finished → intro.
  view.update({
    ...state, phase: 'finished', mode: 'solo', winner: 'you', total: 24,
    mistakes: 0, hints: 0, relinks: 0,
    history: [{word: 'りんご', icon: '🍎', owner: 'you', readings: ['りんご']}],
    chain: {you: {best: 1, bonus: 0}, cpu: {best: 0, bonus: 0}},
  });
  view.update({...state, phase: 'intro', mode: 'solo', cards: []});
