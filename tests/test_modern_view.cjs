...
test('practice hint uses dedicated nr-hint affordance (not muted nr-setting)', () => {
  ...
  // #108: short max-height:500 chip restores ≥44px tap (was 36); keep 11px type.
  assert.match(css, /\.nr-stage-middle > \.nr-hint \{ position: absolute;[\s\S]*?min-height: 44px;[\s\S]*?font-size: 11px !important/);
  // #109: short max-height:500 toolbar pause/help ≥44 (was 34) for cascade safety.
  assert.match(css, /@media \(max-height: 500px\)[\s\S]*?\.nr-toolbar \.nr-button \{ min-height: 44px;/);
  assert.match(css, /@media \(max-height: 500px\)[\s\S]*?\.nr-icon-button \{ min-width: 44px;/);
  // #110: short max-height:500 countdown control ≥44 (was 36) for cascade safety.
  assert.match(css, /@media \(max-height: 500px\)[\s\S]*?\.nr-count-card \.nr-button \{ margin-top: 10px; min-height: 44px; \}/);
...
test('numbers play HUD localizes TIME label to タイム (parity with result #105)', () => {
  const js = fs.readFileSync(require.resolve('../modern-ui.js'), 'utf8');
  // Play stats build uses Japanese タイム; result screen already did.
  assert.match(js, /stat\('タイム',\s*elapsed\)/);
  assert.match(js, /stat\('タイム',\s*resultTime\)/);
  assert.doesNotMatch(js, /stat\('TIME',\s*elapsed\)/);
  assert.doesNotMatch(js, /stat\('TIME'/);
